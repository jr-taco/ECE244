...
void addEmployee(stringstream& lineStream) {
  ...
  Employee* newemployee = new Employee(firstName, lastName, ID, age, salary);
  nameTree->insert(newemployee);
  newemployee = new Employee(firstName, lastName, ID, age, salary);
...
void searchIDEmployee(stringstream& lineStream) {
  ...
  Employee* result = IDTree->searchID(ID);
  if (result) {
    result->print();
  } else {
